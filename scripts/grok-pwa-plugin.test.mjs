...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-host-title-")),
  });
...
test("renders install page markup", () => {
  ...
  assert.match(html, /Add CourtOps Playbook to your/);
...
test("manifest uses site title instead of Grok App on preview hosts", () => {
  const manifest = JSON.parse(
    renderWebManifest("localhost:8080", { site: { title: "CourtOps Playbook" } }),
  );
  assert.equal(manifest.name, "CourtOps Playbook");
  assert.equal(manifest.short_name, "CourtOps Playbook");
});
